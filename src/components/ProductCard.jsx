...
function ProductCard(props) {
  ...
  const addToCart = (values) => {
    ...
    Notification.success("Đã thêm vào giỏ hàng", `${values.name} (${values.sizeOption})`);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
          <Formik
            initialValues={{
              id: props.id || "",
              name: props.name || "",
              image: props.image,  // props.image là mảng các URL string
              sizeOption: "",
              price: 0,
              toppings: [],
              quantity: 1,
          ...
          >
            {({ values, setFieldValue }) => {
              useEffect(() => {
                const selectedSize = props.sizeOptions.find(opt => opt.size === values.sizeOption);
                const sizePrice = selectedSize ? selectedSize.price : 0;
                const toppingsPrice = values.toppings.reduce((sum, t) => sum + t.extraPrice, 0);
                ...
                <Form className="bg-white p-6 rounded shadow-md w-[700px] space-y-4">
                  <div className="flex justify-start">
                    ...
                    <div className="w-2/3 ml-4">
                      ...
                      {(() => {
                        const selected = props.sizeOptions.find(opt => opt.size === values.sizeOption);
                        return (
                      ...
                      {(() => {
                        const selectedSize = props.sizeOptions.find(opt => opt.size === values.sizeOption);
                        const toppingsTotal = values.toppings.reduce((sum, t) => sum + t.extraPrice, 0);
                        const total = selectedSize ? (selectedSize.price + toppingsTotal) * values.quantity : 0;
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex gap-4">
                      {props.sizeOptions.map((item) => (
                        <label key={item.size} className="flex items-center gap-2">
                          <Field
                            type="radio"
                            name="sizeOption"
                            value={item.size}
                          ...
                          />
                          <span>{item.size} - <span className="text-camel font-semibold">{item.price.toLocaleString()}đ</span></span>
                        </label>
                      ))}
                    </div>
                    <ErrorMessage
                  ...
                  {/* Chọn Sugar */}
                  <div className="mb-4">
                    <label className="block font-semibold mb-1">Chọn Mức đường</label>
                    <div className="flex gap-4">
                      ...
                      {["Chung", "Riêng"].map((option) => (
                        <label key={option} className="flex items-center gap-2">
                          <Field
                            ...
                            className="accent-green_starbuck"
                          />
                          <span>{option === "Chung" ? "Đá Chung" : "Đá Riêng"}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block font-semibold mb-1">Chọn Topping</label>
